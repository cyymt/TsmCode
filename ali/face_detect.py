...
parser.add_argument('--caffe_prototxt_path', default="./dingding_ulfd_slim_20201208_dingding_diku.prototxt", type=str, help='caffe_prototxt_path')
parser.add_argument('--caffe_model_path', default="./dingding_ulfd_slim_20201208_dingding_diku.caffemodel", type=str, help='caffe_model_path')
parser.add_argument('--input_size', default="256,192", type=str, help='define network input size,format: width,height')
parser.add_argument('--threshold', default=0.05, type=float, help='score threshold')
parser.add_argument('--imgs_path', default="/grunt/steven.wf/DingdingTest/native_test", type=str, help='imgs dir')
...
def inference():
    ...
    input_size = [int(v.strip()) for v in args.input_size.split(",")]# "256,192"
    witdh = input_size[0]
    height = input_size[1]
    ...
    for file_path in listdir:
        ...
        img_path = '1585273443020_Android.jpg'
        ...
        # 加入输出的文件  后处理 ############
        scores = np.loadtxt('jiaping_scores.txt')
        boxes = np.loadtxt('jiaping_boxes.txt')
        boxes = boxes
        boxes = np.expand_dims(np.reshape(boxes, (-1, 4)), axis=0)
        scores = np.expand_dims(np.reshape(scores, (-1, 2)), axis=0)
        print("shape:", scores.shape, boxes.shape) 
        ...
        iSum = index + iError
        print("index, iError,iSum:", index, iError, iSum)
